...
import os
from drawLandmarks import show
from datetime import date
from tkinter import messagebox
import sys
import random
from inference_classifier import sign_detect
from Voice_to_sign import rr_main
    ...
    while True:
        ...
        if results:
            for i in results:
                ...
                window2.geometry("500x500+520+170")
                window2.resizable(False, False)
                label3 = Label(f1, text="Welcome To SmartVoice", font=("arial", 20, "bold"), bg="grey16", fg="white",
                               relief=SUNKEN)
                ...
                if __name__ == "__main__":
                    l = AnimatedGIF(f1, "files/Insane.gif")
                    l.pack()
                ...
                btn2w2 = ttk.Button(f1, text="Predict Sign", command=sign_detect)
                btn2w2.place(x=25, y=115, width=150, height=40)

                btn3w2 = ttk.Button(f1, text="Show Landmarks", command=test)
                btn3w2.place(x=320, y=115, width=150, height=40)

                btn5w2 = ttk.Button(f1, text="Test", command=show)
                btn5w2.place(x=25, y=225, width=150, height=40)

                btn6w2 = ttk.Button(f1, text="Sound Recognition", command=rr_main)
                btn6w2.place(x=320, y=225, width=150, height=40)


                # =========================Developers Page=========================================

                label10 = Label(f4, text="", font=("arial", 20, "bold"), backg="#000000", fg="white")
                label10.pack(side=TOP, fill=X)
                label11 = Label(f4, text="               Indian Sign Language Recognition System", font=("arial", 10, "bold"), bg="#000000",
                                fg="white")
                label11.pack(side=BOTTOM, fill=X)

                label10 = Label(f4, text=" Project Members Are", font=("arial", 12, "bold"))
                label10.place(x=75, y=110)

                #Members Name
                label10 = Label(f4, text=" Chinmay Hegde", font=("times new roman", 14, "bold"))
                label10.place(x=75, y=160)
                label10 = Label(f4, text=" Adithya R Pai", font=("times new roman", 14, "bold"))
                label10.place(x=75, y=190)
                label10 = Label(f4, text=" Rakshita S", font=("times new roman", 14, "bold"))
                label10.place(x=75, y=220)
                label10 = Label(f4, text=" Abhaya Simha S P", font=("times new roman", 14, "bold"))
                label10.place(x=75, y=250)
                backg = PhotoImage("files/Insane.gif")
                ...
                btn9w2.place(x=25, y=335, width=150, height=40)

                def quit():
                    window2.destroy()

                btn9w2 = ttk.Button(f1, text="Exit", command=quit)
                btn9w2.place(x=320, y=335, width=150, height=40)
